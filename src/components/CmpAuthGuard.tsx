  ...
  if (isLogin) {
    return (<>
      <div className="text-right">
        <button className="btn btn-outline-secondary" onClick={handleLogout}>ログアウト</button>
      </div>
      {children}
    </>);
  }
          ...
          <div className="col-md-8">
            <div className="card">
              <div className="card-body">
                <h5>みずとり会 ログイン</h5>
                <input className="form-control" type="password" id="inputPassword" placeholder="パスワード" />
                <div className="invalid-feedback d-block">{isLoginFail && 'ログインに失敗しました'}</div>
                <div className="row mt-4 justify-content-center">
                  <div className="col-md-6 col-8">
                    <button className="btn btn-block btn-outline-main" id="loginButton" onClick={handleLogin}>ログイン</button>
                  </div>
                </div>
              </div>
            </div>
